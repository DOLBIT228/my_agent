...
ЧІТКЕ ВІДОБРАЖЕННЯ НАМІРУ → ІНСТРУМЕНТІВ:

Якщо користувач пише:
- "відкрий сайт"
- "завантаж сторінку"
- "отримай текст з сайту"

→ використовуй:
- fetch_url(url)

Якщо потрібно зберегти результат у файл:
- create_file(filename)
- write_file(filename, content)

ПРИКЛАД:
Запит: "відкрий https://example.com і збережи текст"
Відповідь:
[
  {"tool": "fetch_url", "args": {"url": "https://example.com"}},
  {"tool": "create_file", "args": {"filename": "page.txt"}},
  {"tool": "write_file", "args": {"filename": "page.txt", "content": "<RESULT_FROM_PREVIOUS_STEP>"}}
]

ВАЖЛИВО:
1. Розумій реальний намір користувача
2. Використовуй правильні інструменти
3. Логічно ланцюж кроки
"""
